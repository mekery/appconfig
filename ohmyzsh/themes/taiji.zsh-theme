# Taiji theme of oh-my-zsh
# Inspired by Zeta theme
# Copyright: Mekery, 2016
# Test: on Linux, Unix and Windows under ANSI colors.
...
ZSH_THEME_GIT_PROMPT_SHA_AFTER="%{$green%}●%{$reset_color%} "
...
function get_git_prompt {
    if [[ -n $(git rev-parse --is-inside-work-tree 2>/dev/null) ]]; then
        ...
        local git_prompt=" %{$yellow%}❆ $(git_prompt_info)$git_status"
        echo $git_prompt
    fi
}
...
function print_prompt_head {
    local left_prompt="\
%{$blue_bold%}$prompt_tips \
%{$green_bold%}$(get_usr_name)@$(get_box_name): \
%{$blue%}$(get_current_dir)%{$reset_color%}\
$(get_git_prompt) "
...
function get_prompt_indicator {
    if [[ $? -eq 0 ]]; then
        echo "%{$blue_bold%}$prompt_taiji %{$reset_color%}"
    else
        echo "%{$red_bold%}$prompt_taiji %{$reset_color%}"
    fi
}
